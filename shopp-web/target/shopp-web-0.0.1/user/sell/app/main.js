var onlyOpenTitle = "首页", _menus, usermenus = undefined, web_version = '1_1_0';
...
function createFrame(url) {
    ...
    return '<iframe scrolling="no"  marginheight="0" marginwidth="0" frameborder="0"  src="/user/sell/viwe/' + url + '" style="width:100%;height:100%;"></iframe>';
}
...
function logout() {
    $.messager.confirm('提示', '确认退出当前系统?', function (r) {
        if (r) {
            $.ajax({
                url: '/i/user/logout.do', type: 'GET', success: function (data) {
                    codeuser = undefined;
                    window.sessionStorage.clear();
                    top.location.href = window.location.origin+"/user/login.html";
                }
            });
        }
    });
...
function checkuser() {
    $.ajax({
        type: "GET", cache: false, dataType: 'json', url: "/i/user/isLogin.do", success: function (data) {
            if(200==data.status){
                window.user=data.entity;
            }else {
                $.messager.alert('警告', '当前会话已失效！请重新登录！', 'error');
                codeuser = undefined;
                window.sessionStorage.clear();
                var url = window.location.origin + "/user/login.html";
...
$().ready(function () {
    if(window.sessionStorage.asikey){
        $.ajax({
            type: "GET", cache: false, dataType: 'json', url: "/i/user/getUserMenu.do", success: function (data) {
                usermenus = data;
                if (!usermenus) {
                    window.sessionStorage.clear(); top.location.href = "login.html";
                }
                tabClose();
                tabCloseEven();
                Time();
